...
namespace ECS
{
    ...
    public partial class EcsWorld
    {
        ...
        private int m_UsedComponentsCount;

        internal readonly EcsWorldConfig Config;
        private readonly object[] m_FilterCtor;
        ...
        private readonly EcsGrowList<EcsEntity> m_LeakedEntities = new EcsGrowList<EcsEntity>(256);
        private bool m_InDestroying;
        ...
        public EcsFilter GetFilter(Type filterType, bool createIfNotExists = true)
        {
            ...
            // create new filter.
            EcsFilter filter = (EcsFilter)Activator.CreateInstance(filterType, BindingFlags.NonPublic | BindingFlags.Instance, null, m_FilterCtor, CultureInfo.InvariantCulture);
#if DEBUG
        ...
        public EcsWorldStats GetStats()
        {
            EcsWorldStats stats = new EcsWorldStats()
            {
                ...
                Components = m_UsedComponentsCount
            };
            return stats;
        }
        ...
        protected internal void RecycleEntityData(int id, ref EcsEntityData entityData)
        {
            ...
            if (entityData.ComponentsCountX2 != 0)
            {
                throw new Exception("Cant recycle invalid entity.");
            }
#endif
        ...
        public bool CheckForLeakedEntities(string errorMsg)
        {
            if (m_LeakedEntities.Count > 0)
            {
                ...
                {
                    if (GetEntityData(m_LeakedEntities.Items[i]).ComponentsCountX2 == 0)
                    {
                        if (errorMsg != null)
                        {
            ...
            if (typeIdx < 0)
            {
                ...
                if (FilterByIncludedComponents.TryGetValue(-typeIdx, out filters))
                {
                    for (int i = 0, iMax = filters.Count; i < iMax; i++)
                    {
                        ...
                        {
#if DEBUG
                            if (!filters.Items[i].GetInternalEntitiesMap().TryGetValue(entity.GetInternalId(), out int filterIdx))
                            {
                                filterIdx = -1;
                            }
        ...
        public ref EcsEntityData GetEntityData(in EcsEntity entity)
        {
            ...
            if (entity.Id < 0 || entity.Id > EntitiesCount)
            {
                throw new Exception($"Invalid entity {entity.Id}");
            }
#endif
